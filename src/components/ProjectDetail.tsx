      ...
      <header className="relative pt-32 pb-20 px-6 overflow-hidden border-b border-white/5">
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <h1 className="text-[46px] font-bold tracking-tight mb-4 leading-[1.1]">
              {project.title}
            </h1>
            <p className="text-[18px] text-[#E6E6FD] leading-relaxed font-light">
              {project.desc}
            </p>
          </motion.div>
          ...
          <motion.div 
            ...
            className="w-full max-w-[900px] h-[350px] mx-auto rounded-[2rem] overflow-hidden border border-white/10 relative"
          >
            <img 
          ...
          {project.sections.map((section, index) => (
            ...
            >
              <div className="space-y-6">
                ...
                <div className="w-full h-px bg-[#292929]"></div>
              </div>
                ...
                {Array.isArray(section.content) ? (
                  <div className="space-y-6">
                    {section.content.map((item, i) => {
                      ...
                      const imageRegex = /(https?:\/\/[^\s]+?\.(?:jpg|jpeg|png|gif|webp|svg|avif)(?:\?[^\s]*)?|https:\/\/i?\.imghippo\.com\/files\/[^\s]+)/gi;
                      
                      // Split text by image URLs, keeping the URLs in the result
                      const parts = item.split(imageRegex);
                      
                      if (parts.length === 1) {
                        // Original logic for simple strings
                        const isList = item.startsWith('•') || item.startsWith('*') || item.match(/^\d+\./);
                        return (
                          <p 
                            key={i} 
                            className={`text-[14px] leading-relaxed text-[#E6E6FD] font-light ${isList ? 'pl-6' : ''}`}
                          >
                            {item}
                          </p>
                        );
                      ...
                      return (
                        <div key={i} className="flex flex-col gap-4">
                          {parts.map((part, partIdx) => {
                            const isImgUrl = part.match(imageRegex);
                            if (isImgUrl) {
                              return (
                                <div key={partIdx} className="rounded-2xl overflow-hidden border border-white/5 my-4">
                                  <img 
                                    src={part} 
                            ...
                            const isList = part.trim().startsWith('•') || part.trim().startsWith('*') || part.trim().match(/^\d+\./);
                            return (
                              <p 
                                key={partIdx} 
                                className={`text-[14px] leading-relaxed text-[#E6E6FD] font-light ${isList ? 'pl-6' : ''}`}
                  ...
                  <p className="text-[14px] leading-relaxed text-[#E6E6FD] font-light">
                    {section.content}
                  </p>
                )}
              ...
              {relatedProjects.map((p) => (
                ...
                >
                  ...
                  <div className="p-8 flex-grow flex flex-col">
                    ...
                    <p className="text-[#E6E6FD]/60 text-sm font-light line-clamp-2">{p.desc}</p>
                  </div>
                </Link>
              ))}
            </div>
