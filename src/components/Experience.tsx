...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-[108px] md:py-32 px-6 md:px-32 bg-[#000000] overflow-hidden">
      <div className="text-center mb-32">
        <h2 className="text-[29px] md:text-4xl font-bold mb-6 tracking-tight">职业轨迹</h2>
        ...
        <div className="relative isolate">
          {experiences.map((exp, i) => (
            ...
            >
              ...
              <div className={`w-full md:w-1/2 p-2 md:p-4 relative pointer-events-auto ${exp.side === 'left' ? 'md:pr-12' : 'md:pl-12'}`}>
                <motion.div 
                  initial={{ opacity: 0, x: exp.side === 'left' ? -30 : 30 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
                  className="p-4 md:p-6 rounded-[2rem] md:rounded-[2.5rem] bg-gradient-to-br from-white/[0.05] to-transparent border-2 border-[rgba(var(--accent-rgb),0.5)] group-hover/exp:border-[rgba(var(--accent-rgb),1)] hover:bg-white/[0.07] transition-all duration-300 relative shadow-2xl"
                >
                  <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 ml-0 md:ml-5 mt-2 md:mt-5">
                    <img 
                      src={exp.logo} 
                      alt={exp.company} 
                      className="w-16 h-16 rounded-2xl object-cover bg-white/5 border border-white/10 group-hover/exp:border-blue-500/50 transition-all duration-500 shadow-2xl mb-2 md:mb-0"
                      referrerPolicy="no-referrer"
                    />
                    <div className="flex-1 text-center md:text-left">
                      <div 
                        className="font-bold text-[12px] mb-2 tracking-[3px] uppercase opacity-80"
                        style={{ color: `rgb(var(--accent-rgb))` }}
                      >
                        {exp.date}
                      </div>
                      <h3 
                        className="text-xl md:text-2xl font-bold mb-2 tracking-tight transition-colors group-hover/exp:text-[rgb(var(--accent-rgb))]"
                      >
                        {exp.company}
                      </h3>
                      <div className="text-[#A8A8A8] text-xs font-medium tracking-wide uppercase">{exp.role}</div>
                    </div>
                  </div>
                  
                  <div className="mt-4 p-4 md:p-6 rounded-2xl bg-black/40 backdrop-blur-sm border border-white/5 group-hover/exp:border-white/10 transition-colors">
